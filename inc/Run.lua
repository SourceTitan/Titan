...
Titan = Token:match("(%d+)")
redis:set(Titan..":VERSION",2.9)
redis:set(Titan..":SUDO_ID:",SUDO_USER)
redis:set(Titan..":DataCenter:",'German')
redis:set(Titan..":UserNameBot:",BOT_User)
redis:set(Titan..":NameBot:",BOT_NAME)
redis:hset(Titan..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3Titan_INSTALL","Yes")
info = {}
...
Titan = Token:match("(%d+)")
our_id = tonumber(Titan)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(Titan..":UserNameBot:")
SUDO_ID = tonumber(redis:get(Titan..":SUDO_ID:"))
SUDO_USER = redis:hgetall(Titan..'username:'..SUDO_ID).username
version = redis:get(Titan..":VERSION")
DataCenter = redis:get(Titan..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..Titan..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(Titan..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(Titan..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(Titan..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(Titan..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(Titan..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي👷🏽'
msg.TheRank = 'المنشىء الاساسي👷🏽 '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(Titan..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(Titan..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(Titan..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(Titan..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(Titan..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(Titan..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(Titan..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(Titan..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(Titan..':WELCOME_BOT'),[[🙋🏽‍♂╿ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(Titan..':NameBot:')..[[ ⚜
...
if msg.GroupActive and redis:get(Titan..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(Titan..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(Titan..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(Titan..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.Titan then 
if msg.text and ScriptFile.iTitan then
for k, Titan in pairs(ScriptFile.Titan) do
local SearchText = msg.text:match(Titan)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iTitan(msg,{msg.text:match(Titan)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",Titan.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dTitan then
if ScriptFile.dTitan(msg) == false then
return false
end
...
if Plug.Titan then 
if msg.text and Plug.iTitan then
for k, Titan in pairs(Plug.Titan) do
local SearchText = msg.text:match(Titan)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(Titan..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(Titan..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iTitan(msg,{msg.text:match(Titan)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",Titan.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source Titan")
end 

end
else
print("The File Script.lua Not Runing in The Source Titan")
...
function tdcli_update_callback(data)
	...
	redis:srem(Titan..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/SourceTitan/Titan/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/SourceTitan/Titan/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/SourceTitan/Titan/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/SourceTitan/Titan/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/SourceTitan/Titan/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/SourceTitan/Titan/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(Titan..":SUDO_ID:")
redis:hset(Titan..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(Titan..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(Titan..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(Titan..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(Titan..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(Titan..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(Titan..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(Titan..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(Titan..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(Titan..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(Titan..'group:add'..data.chat_.id_) then
	redis:set(Titan..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(Titan..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(Titan..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(Titan..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	Titan..'group:name'..k,IDS.Title,
	Titan..'num_msg_Titan'..k,5,
	Titan..'group:add'..k,true,
	Titan..'lock_link'..k,true,
	Titan..'lock_spam'..k,true,
	Titan..'lock_webpage'..k,true,
	Titan..'lock_markdown'..k,true,
	Titan..'lock_flood'..k,true,
	Titan..'lock_bots'..k,true,
	Titan..'mute_forward'..k,true,
	Titan..'mute_contact'..k,true,
	Titan..'mute_document'..k,true,
	Titan..'mute_inline'..k,true,
	Titan..'lock_username'..k,true,
	Titan..'replay'..k,true
	)
	redis:sadd(Titan..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(Titan..'username:'..ID,'username',user)
	redis:sadd(Titan..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(Titan..'username:'..ID,'username',user)
	redis:sadd(Titan..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(Titan..'username:'..ID,'username',user)
	redis:sadd(Titan..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(Titan..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(Titan..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(Titan..'username:'..data.user_.id_).username
	...
	redis:hset(Titan..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
