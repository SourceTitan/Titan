...
if GeneralBanned(UserID) then return sendMsg(ChatID,MsgID,'👤 ܛ  العضو » '..namei..'\n🎟 ܛ  الايدي » ❪ `'..UserID..'` ❫\n💯 ܛ  تم بالتأكيد حظره عام \n✓️') end
redis:hset(Titan..'username:'..UserID, 'username',Resolv)
...
if data.status_.ID == "ChatMemberStatusMember" then 
Restrict(ChatID,UserID,1)  
return sendMsg(ChatID,MsgID,'▾╮ المستخدم ⊱ '..namei..'\n🎟 ܛ  الايدي » ❪ `'..UserID..'` ❫\n💯 ܛ  تم تقييد آلعضـو بنجآح \n✓️') 
else
...
if redis:sismember(Titan..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'  ܛ  العضو » '..namei..'\n🎟 ܛ  الايدي » ❪ `'..UserID..'` ❫\n▾╮ انه بالتأكيد ادمن \n✓️') 
end
redis:hset(Titan..'username:'..UserID, 'username', UserName)
redis:sadd(Titan..'admins:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'▾╮ المستخدم ⊱ '..namei..'\n🎟 ܛ  الايدي » ❪ `'..UserID..'` ❫\n💯 ܛ  تمت ترقيته ليصبح ادمن \n✓️')
...
if cmd == "unban" then
if not redis:sismember(Titan..'banned:'..ChatID,UserID) then 
sendMsg(ChatID,MsgID,'▾╮ المستخدم ⊱ '..namei..'\n ▾╮ الايدي ⊱ ❪ `'..UserID..'` ❫\n💯 ܛ  تم بالتأكيد الغاء حظره \n✓️') 
else
sendMsg(ChatID,MsgID,'▾╮ المستخدم ⊱ '..namei..'\n ▾╮ الايدي ⊱ ❪ `'..UserID..'` ❫\n💯 ܛ  تم الغاء حظره \n✓️')
end
redis:srem(Titan..'banned:'..ChatID,UserID)
...
if cmd == "dn_sudo" then
if not redis:sismember(Titan..':SUDO_BOT:',UserID) then return sendMsg(ChatID,MsgID,'▾╮ المستخدم ⊱ '..namei..' \n ▾╮ الايدي ⊱ (`'..UserID..'`)\n▾╮ انه بالتأكيد ليس مطور \n✓') end
redis:srem(Titan..':SUDO_BOT:',UserID) 
return SendMention(ChatID,UserID,MsgID,'▾╮ المستخدم ⊱ '..namei..'\n ▾╮ الايدي ⊱ '..UserID..'\n▾╮ تم تنزيله من المطورين \n✓️',17,USERCAR) 
end
...
if not Check_Banned(ChatID,UserID) then 
 SendMention(ChatID,UserID,MsgID,'▾╮ المستخدم ⊱ '..namei..'\n ▾╮ الايدي ⊱ '..UserID..'\n▾╮ تم بالتأكيد الغاء حظره \n✓️',17,USERCAR) 
else
 SendMention(ChatID,UserID,MsgID,'▾╮ المستخدم ⊱ '..namei..'\n ▾╮ الايدي ⊱ '..UserID..'\n▾╮ تم الغاء حظره \n✓️',17,USERCAR) 
end
redis:hset(Titan..'username:'..UserID, 'username', Resolv)
